<script>
  ...
  function captureBookbox(elementSelector = '#bookbox', filename = 'bookshelf.png') {
...
<div class="md:w-8/12 p-4 mx-auto">
  ...
  <div id="bookbox" class="flex justify-center items-end border-[24px] border-[#888888] p-[12px] w-[689px] h-[429px]">
  ...
  <button id="download" class="bg-black text-white px-4 py-2 rounded mt-4" onclick={() => captureBookbox('#bookbox')}>
    本箱の画像をダウンロード
  </button>

  <div>
  {#each [...books].reverse() as book, i (book.isbn)}
    {book.book.author.replace(/\s*\[*著\]*$/, '')}『{book.book.title}』{book.book.label}、{book.book.date.slice(0, 4)}年。<br />
  {/each}
  </div>

  <div class="mt-4">
    {#each [...books].reverse() as book, i (book.isbn)}
      {book.book.author.replace(/\s*\[*著\]*$/, '')}『{book.book.title}{book.book.subtitle ? `──${book.book.subtitle}` : ''}』{book.book.label}、{book.book.date.slice(0, 4)}年。<br />
    {/each}
  </div>
